...
.left-panel::after{
  ...
  background:
    radial-gradient(ellipse 65% 55% at 25% 20%, rgba(201,168,76,0.12) 0%,transparent 65%),
    radial-gradient(ellipse 45% 35% at 85% 80%, rgba(201,168,76,0.06) 0%,transparent 60%);
...
/* floating literary quotes */
.q{
  position:absolute;z-index:3;
  font-family:'Cormorant Garamond',serif;
  font-style:italic;color:rgba(201,168,76,0.2);
  line-height:1.6;pointer-events:none;
  animation:drift var(--dur,18s) ease-in-out infinite;
  animation-delay:var(--delay,0s);
}
@keyframes drift{
  0%,100%{transform:translateY(0) rotate(var(--rot,-1deg));opacity:0.45;}
  40%{transform:translateY(-14px) rotate(calc(var(--rot) * -0.5));opacity:0.9;}
  70%{transform:translateY(-8px) rotate(var(--rot));opacity:0.65;}
}
...
/* animated book spines at bottom */
.spine-shelf{
  position:absolute;bottom:0;left:0;right:0;z-index:3;
  display:flex;align-items:flex-end;gap:3px;padding:0 20px;height:90px;overflow:hidden;
}
.sp{
  border-radius:2px 2px 0 0;flex-shrink:0;
  animation:spineRise 1s ease-out both;
...
/* progress tracker */
.prog-track{display:flex;align-items:center;margin-bottom:28px;}
.prog-step{
...
.prog-step.active .prog-dot{background:var(--gold);border-color:var(--gold);color:#0d0d0d;}
.prog-step.done .prog-dot{background:#4a7c59;border-color:#4a7c59;color:#fff;}
.prog-line{flex:1;height:1px;background:var(--border);margin:0 7px;}
...
.err-box{
  ...
  color:#e05c5c;font-size:0.77rem;padding:10px 13px;border-radius:7px;margin-bottom:20px;
}
...
.fg .check-mark{
  ...
  color:#4a7c59;font-size:0.78rem;opacity:0;transition:opacity 0.2s;
}
...
.btn-go{
  ...
  border:none;border-radius:8px;padding:14px;margin-top:10px;
...
<div class="left-panel">
  ...
  <!-- floating quotes -->
  <div class="q" style="top:7%;left:5%;font-size:0.82rem;--dur:20s;--delay:0s;--rot:-1.5deg;max-width:200px;">
    "A reader lives a thousand<br>lives before he dies."
  </div>
  <div class="q" style="top:20%;right:3%;font-size:0.7rem;--dur:17s;--delay:5s;--rot:1deg;max-width:180px;text-align:right;">
    "Books are a uniquely<br>portable magic."<br><span style="font-size:0.6rem;opacity:0.6;">— Stephen King</span>
  </div>
  <div class="q" style="bottom:32%;left:4%;font-size:0.75rem;--dur:22s;--delay:9s;--rot:-1deg;max-width:190px;">
    "I declare after all there is no<br>enjoyment like reading!"
  </div>
  <div class="q" style="bottom:18%;right:5%;font-size:0.68rem;--dur:19s;--delay:3s;--rot:1.2deg;max-width:160px;text-align:right;">
    "So many books,<br>so little time."
  </div>

  <div class="panel-inner">
    <div class="p-eyebrow">Join E-Library Today</div>
    <h1 class="p-title">Your next great<br>read is waiting<br>for <em>you.</em></h1>
    <p class="p-desc">Join thousands of readers across Pakistan who have already discovered the joy of unlimited digital reading.</p>

    <div class="p-sep"><div class="p-sep-dot"></div></div>

    <div class="feat">
      <div class="feat-ic" style="background:rgba(201,168,76,0.1);color:#c9a84c;"><i class="fa-solid fa-bolt"></i></div>
      <span>Instant PDF access after purchase</span>
    </div>
    <div class="feat">
      <div class="feat-ic" style="background:rgba(74,124,89,0.12);color:#6abd7c;"><i class="fa-solid fa-book-open"></i></div>
      <span>Hundreds of free books available</span>
    </div>
    <div class="feat">
      <div class="feat-ic" style="background:rgba(99,102,241,0.12);color:#818cf8;"><i class="fa-solid fa-trophy"></i></div>
      <span>Participate in essay competitions</span>
    </div>
    <div class="feat">
      <div class="feat-ic" style="background:rgba(224,92,92,0.1);color:#f87171;"><i class="fa-solid fa-mobile-screen"></i></div>
      <span>Read on any device, anywhere</span>
    </div>
  </div>

  <!-- book spine shelf at bottom -->
  ...
  <div class="form-wrap">
    ...
    <div class="prog-track" id="progTrack">
      <div class="prog-step active" id="ps1">
        <div class="prog-dot" id="pd1">1</div>Account
      </div>
      <div class="prog-line"></div>
      <div class="prog-step" id="ps2">
        <div class="prog-dot" id="pd2">2</div>Contact
      </div>
      <div class="prog-line"></div>
      <div class="prog-step" id="ps3">
        <div class="prog-dot" id="pd3">3</div>Security
      </div>
    </div>
    ...
    <form method="POST" id="regForm">
      ...
      <div class="fg">
        ...
        <input type="text" name="phone" id="fp" class="fi" placeholder="Phone Number" required autocomplete="tel">
        <label class="fl" for="fp">Phone Number</label>
        <i class="fa-solid fa-check check-mark"></i>
      </div>
      ...
      <div class="fg">
        ...
        <button type="button" class="pwd-btn" onclick="toggleP()" id="eyeBtn">
...
/* password show/hide */
function toggleP(){
  var i=document.getElementById('fpass');
...
/* strength meter */
function checkStr(v){
  var w=document.getElementById('strWrap');
  var l=document.getElementById('strLbl');
  var bs=[document.getElementById('sb1'),document.getElementById('sb2'),
          document.getElementById('sb3'),document.getElementById('sb4')];
  bs.forEach(function(b){b.className='sb';});
  ...
  var lvl=[{c:'w',t:'Weak — keep going'},{c:'f',t:'Fair — getting better'},{c:'g',t:'Good — almost there'},{c:'s',t:'Strong — great!'}];
  for(var i=0;i<sc;i++)bs[i].classList.add(lvl[sc-1].c);
  l.textContent=lvl[sc-1].t;
}

/* step progress on focus */
var fMap={fn:1,fe:2,fp:2,fpass:3};
document.querySelectorAll('.fi').forEach(function(el){
  el.addEventListener('focus',function(){
    var n=fMap[this.id]||1;
    [1,2,3].forEach(function(i){
      var ps=document.getElementById('ps'+i);
      var pd=document.getElementById('pd'+i);
      if(i<n){
        ps.className='prog-step done';
        pd.innerHTML='<i class="fa-solid fa-check" style="font-size:0.5rem;"></i>';
      } else if(i===n){
